...
public class StringUtils {
	...
	public static String formatExtendedTime(double time) {
		boolean negative = false;
		if (time < 0) {
			negative = true;
			time *= -1;
		...
		int minutes = (int) (time - (hours * 3600)) / 60;
		int seconds = (int) time - (hours * 3600) - (minutes * 60);
		return (hours != 0 ? (negative ? "-" : "") + hours + " hour" + (hours == 1 ? "" : "s") : "") + (minutes != 0 ? (negative ? "-" : "") + minutes + " minute" + (minutes == 1 ? "" : "s") : "") + (seconds != 0 ? (negative ? "-" : "") + seconds + " second" + (seconds == 1 ? "" : "s") : "");
	}
